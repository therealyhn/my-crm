...
```env
VITE_API_BASE_URL=http://localhost/Projects/my-crm/backend/api
```
...
1. `backend/migrations/001_create_core_schema.sql`
2. `backend/migrations/002_create_notifications.sql`
3. `backend/migrations/003_add_client_project_meta_fields.sql`

Optional rollback helper:

- `backend/migrations/999_drop_core_schema.sql`

### 5) Run backend on Apache

Use project path so API resolves, for example:

- `http://localhost/Projects/my-crm/backend/api/health`

If health returns OK, frontend + backend wiring is ready.
